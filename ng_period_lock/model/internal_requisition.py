...
class internal_requisition(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        p_lock_id = self.pool.get("period.lock").search(cr, uid, [('model_str','=','internal.requisition')],context=context)
            ...
            if uid in lock_user or uid in lock_group_user:
                if rec.create == True:
                    if rec.lock_end_date:
                        ...
                        raise osv.except_osv(('Warning'),('%s')%(warning,))
        return super(internal_requisition, self).create(cr, uid, vals, context=context)
            ...
            if uid in lock_user or uid in lock_group_user:
                if rec.delete == True:
                    if rec.lock_end_date:
                        ...
                        raise osv.except_osv(('Warning'),('%s')%(warning,))
        return super(internal_requisition,self).unlink(cr, uid, ids, context=context)
            ...
            if uid in lock_user or uid in lock_group_user:
                if rec.duplicate == True:
                    if rec.lock_end_date:
                        ...
                        raise osv.except_osv(('Warning'),('%s')%(warning,))
        return super(internal_requisition,self).copy(cr, uid, id, default=default, context=context)
