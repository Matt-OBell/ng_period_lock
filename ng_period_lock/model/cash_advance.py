...
class cash_advance(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        p_lock_id = self.pool.get("period.lock").search(cr, uid, [('model_str','=','cash.advance')],context=context)
            ...
            if uid in lock_user or uid in lock_group_user:
                if rec.create == True:
                    if rec.lock_end_date:
                        ...
                        raise osv.except_osv(('Warning'),('%s')%(warning,))
        return super(cash_advance, self).create(cr, uid, vals, context=context)
            ...
            if uid in lock_user or uid in lock_group_user:
                if rec.write == False and lock_state and new_state:
                    ...
                    if rec.lock_end_date:
                        ...
                        raise osv.except_osv(('Warning'),('%s')%(warning,))
        return super(cash_advance, self).write(cr, uid, ids, vals, context=context)
            ...
            if uid in lock_user or uid in lock_group_user:
                if rec.duplicate == True:
                    if rec.lock_end_date:
                        ...
                        raise osv.except_osv(('Warning'),('%s')%(warning,))
        return super(cash_advance,self).copy(cr, uid, id, default=default, context=context)
