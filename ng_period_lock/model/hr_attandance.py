...
class hr_attendance(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        p_lock_id = self.pool.get("period.lock").search(cr, uid, [('model_str','=','hr.attendance')],context=context)
            ...
            if uid in lock_user or uid in lock_group_user:
                if rec.duplicate == True:
                    if rec.lock_end_date:
                        ...
                        raise osv.except_osv(('Warning'),('%s')%(warning,))
        return super(hr_attendance,self).copy(cr, uid, id, default=default, context=context)
