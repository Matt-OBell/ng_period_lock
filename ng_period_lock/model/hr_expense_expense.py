...
class hr_expense_expense(osv.osv):
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        p_lock_id = self.pool.get("period.lock").search(cr, uid, [('model_str','=','hr.expense.expense')],context=context)
            ...
            if uid in lock_user or uid in lock_group_user:
                if rec.delete == True:
                    if rec.lock_end_date:
                        ...
                        raise osv.except_osv(('Warning'),('%s')%(warning,))
        return super(hr_expense_expense,self).unlink(cr, uid, ids, context=context)
